...
const THEME_KEY_PREFIX = "customer_support_theme";
const AVATAR_KEY_PREFIX = "customer_support_avatar";
...
function App() {
  ...
  const [profilePreferences, setProfilePreferences] = useState<
    Record<string, ProfilePreferences>
  >({});
  ...
  const [activePage, setActivePage] = useState<
    ...
    | "approvals"
  >("conversation");
  const profileKey = customerId || username;
  const profileStorageKey = (prefix: string) =>
    `${prefix}:${profileKey.toLowerCase()}`;
  const storedProfilePreferences = profilePreferences[profileKey];
  const theme = storedProfilePreferences?.theme || "light";
  const profileAvatar = storedProfilePreferences?.avatar || "";
  ...
  const updateTheme = (nextTheme: ThemeName) => {
    setProfilePreferences((previous) => ({
      ...
      [profileKey]: {
        theme: nextTheme,
        avatar: previous[profileKey]?.avatar || "",
      },
    }));
    if (accessToken && profileKey) {
      localStorage.setItem(profileStorageKey(THEME_KEY_PREFIX), nextTheme);
    }
  };
  ...
  const updateProfileAvatar = (avatar: string) => {
    setProfilePreferences((previous) => ({
      ...previous,
      [profileKey]: {
        theme: previous[profileKey]?.theme || "light",
        avatar,
      },
    }));
    if (accessToken && profileKey) {
      localStorage.setItem(profileStorageKey(AVATAR_KEY_PREFIX), avatar);
    }
  };

  const hydrateProfilePreferences = (identifier: string) => {
    if (!identifier || identifier === "guest") {
      return;
    }
    const normalizedIdentifier = identifier.toLowerCase();
    const storedTheme = localStorage.getItem(
      `${THEME_KEY_PREFIX}:${normalizedIdentifier}`,
    );
    const storedAvatar = localStorage.getItem(
      `${AVATAR_KEY_PREFIX}:${normalizedIdentifier}`,
    );
    setProfilePreferences((previous) => ({
      ...previous,
      [identifier]: {
        theme:
          storedTheme === "ocean" || storedTheme === "sunset"
            ? storedTheme
            : "light",
        avatar: storedAvatar || "",
      },
  ...
  useEffect(() => {
    ...
    const restoreSession = async () => {
      try {
        ...
        hydrateProfilePreferences(data.customer_id || data.username);

        setShowLogin(false);

        void loadNotifications(storedToken).catch((error) =>
          console.warn("[NOTIFICATIONS] Could not restore notifications:", error),
        );
  ...
  const handleLogin = async (
    ...
    try {
      ...
      hydrateProfilePreferences(configData.customer_id || configData.username);

      setShowLogin(false);
      setLoginPassword("");
      setShowPassword(false);
      setLoginError("");
      void loadNotifications(data.access_token).catch((error) =>
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      if (accessToken) {
        void loadNotifications(accessToken).catch((error) =>
          console.warn("[NOTIFICATIONS] Could not refresh notifications:", error),
        );
      }
