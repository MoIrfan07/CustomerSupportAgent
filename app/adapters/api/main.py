...
NOTIFICATIONS_PATH = (
    Path(__file__).resolve().parents[3] / "data" / "notifications.json"
)
...
async def notifications(current_user: User = Depends(get_current_user)):
    ...
    try:
        ...
        logger.exception("NOTIFICATIONS LOAD FAILED | user=%s", current_user.username)
        raise HTTPException(
            status_code=500,
            detail="Notifications could not be loaded.",
        ) from error
    return {
        "notifications": data.get(current_user.customer_id, [])[-20:][::-1],
    }
